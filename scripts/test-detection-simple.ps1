...
Write-Host "Salesforce Metadata Detection Test" -ForegroundColor Cyan
...
if ([int]$commitCount -gt 1) {
    ...
    if ($strategy1Files) {
        Write-Host "SUCCESS: Found $($strategy1Files.Count) files:" -ForegroundColor Green
...
if ($originMainExists) {
    ...
    if ($strategy2Files) {
        Write-Host "SUCCESS: Found $($strategy2Files.Count) files:" -ForegroundColor Green
...
if ($strategy3Files) {
    Write-Host "SUCCESS: Found $($strategy3Files.Count) files in force-app" -ForegroundColor Green
...
$changedFiles = @()
if ($strategy1Files -and $strategy1Files.Count -gt 0) {
    $changedFiles = $strategy1Files
    Write-Host ""
    Write-Host "RESULT: Using Strategy 1 results ($($strategy1Files.Count) files)" -ForegroundColor Green
} elseif ($strategy2Files -and $strategy2Files.Count -gt 0) {
    $changedFiles = $strategy2Files
    Write-Host ""
    Write-Host "RESULT: Using Strategy 2 results ($($strategy2Files.Count) files)" -ForegroundColor Green
} elseif ($strategy3Files -and $strategy3Files.Count -gt 0) {
    $changedFiles = $strategy3Files
    Write-Host ""
    Write-Host "RESULT: Using Strategy 3 results ($($strategy3Files.Count) files)" -ForegroundColor Green
} else {
    Write-Host ""
...
if ($apexFiles -and $apexFiles.Count -gt 0) {
    Write-Host ""
...
$lwcComponents = @()
if ($lwcFiles -and $lwcFiles.Count -gt 0) {
...
$auraComponents = @()
if ($auraFiles -and $auraFiles.Count -gt 0) {
...
if ($otherMetadata -and $otherMetadata.Count -gt 0) {
    Write-Host ""
...
Write-Host "- LWC components: $($lwcComponents.Count)"
Write-Host "- Aura components: $($auraComponents.Count)"
Write-Host "- Static resources: $($staticResources.Count)"
...
if ($apexDirs -and $apexDirs.Count -gt 0) {
    Write-Host ""
...
if ($lwcComponents -and $lwcComponents.Count -gt 0) {
    Write-Host ""
...
if ($auraComponents -and $auraComponents.Count -gt 0) {
    Write-Host ""
...
if ($staticResources -and $staticResources.Count -gt 0) {
    Write-Host ""
